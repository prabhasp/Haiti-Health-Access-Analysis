...
Below, the more purple the regions, the higher the distance to a health facility.
...
health_df <- subset(health_df, select=c("x", "y", "row", "Categorie"))
...
```
Here is another view; each line links a population point (every 20km) to a health facility. Long lines mean long distances to the health facility. Inspired by [[1]](http://flowingdata.com/2013/08/27/in-search-of-food-deserts/).  
...
Here, distance to health facility and population at point of origin are shown *together*.
Darker (more purple) indicate bigger populations; long dark lines are larger populations who have to travel long distances to health facilities.
...
Last but not the least, it is worth noting that not all health facilities are made the same, and some health facilities are serving huge amounts of people. Here is a look at this phenomenon, by category of health facility:
...
ggplot(data=popServed, aes(x=x, y=y, size=popServed, color=Categorie)) + geom_point() # + blank_theme()
```

The larger red and green points, dispensaries and health centers serving large amounts of people, are worth thinking hard about.
