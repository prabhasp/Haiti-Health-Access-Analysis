...
require(maptools)
admin_regions <- readShapeSpatial('data/Haiti_Sections_Final_WGS84.shp', proj4string=wgs84)
admin_regions_DF <- fortify(spTransform(admin_regions, utm))
...
require(ggmap)
gplot(popagg) + geom_tile(aes(fill=value)) + 
...
**A different view:** Lines connect population points (every 2km) to health facilities. Long lines mean long distances to the nearest health facility. (Inspired by [[1]](http://flowingdata.com/2013/08/27/in-search-of-food-deserts/))
...
## 2. Where are there __large__ populations far away from health facilities?
...
#                filename='figure/map.Haiti.png', color='bw', zoom=8), alpha=0.2) +
 geom_point(data=popServed, aes(x=longitude, y=latitude, size=popServed, color=Categorie)) + 
